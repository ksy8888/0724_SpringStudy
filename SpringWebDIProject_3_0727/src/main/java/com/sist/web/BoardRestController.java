 ...
 *       jsp(forward) = request를 전송 / .do (sendRedirect) = request를 전송하지 않고 기존의 파일로 이동(이동하는 화면이 다른 경우)
 *       
 *       detail.do => detail.jsp       insert_ok.do => list.do (insert_ok를 보여주지않고 list로 이동)
...
public class BoardRestController {
	 ...
	 *        ------전송 객체 (forward일 때)
	 *    = request를 사용하지 않는다
	 *      1) request.getParameter() : 매개변수 (DispatcherServlet에서 request.getParameter()한 다음에 매개변수로 값을 전송)
	 *      2) request대신에 Model을 이용해서 request에 담긴 데이터를 JSP전송
	 */
	// => 크롬에서만 가능 ==> 자바스크립트에 데이터를 전송할 경우에 사용
	// Ajax , Vue , React => JSON
	/*
	 * 	Ajax, Vue ,React => BoardVO   List 를 받지 못함
	 * 						  |        |
