...
<body>
	<div class="container">
	 <h1 class="text-center">VueJS(디렉티브: v-for)</h1>
	 <div class="row">
	  <table class="table">
	   <tr>
	    <td>
	      <input type=text size=30 class="input-sm" v-model="msg" ref="msg">
	      <input type=button class="btn btn-sm btn-primary" value="입력" v-on:click="find()">
	    </td>
	   </tr> 
	  </table>
	   <table class="table">
	    ...
	    <tbody>
	      <tr v-for="vo in movie"> <%--vo in 변수명 --%>
	...
	new Vue({
		...
		beforeUpdate:function() {
			...
			//멤버변수 설정 : react => state, Vue => data => data를 다른 메소드에서 변경시에는 this.
		},
		updated:function() {
			console.log("***updated: 데이터변경이나 수정이 완료된 상태 => mounted를 재호출")
		},
		beforeDestroy:function() {
			console.log("beforeDestroy:메모리 해제 전 상태")
		},
		destroyed:function() {
			console.log("destroyed: 화면이 변경 => Vue객체 해제")
		},
		// 사용자 정의 함수
		methods: {
			find: function() {
