...
<body>
	<div class="container-fluid">
	 <div class="row">
	   ...
	   </div> <!-- member/logout.do >> MemberControllers -->
	 </div>
	  <div class="row">
	 ...
	 new Vue({
		...
		methods: {
			categoryGetData:function(cno) {
				axios.get('../food/food_category_vue.do', {	//restController에서 처리
					params:{
						cno:cno	//cno변수 넘김
					}
				}).then(response=> {
			...
			foodGetList:function(cno,bool) {
				...
				axios.get("../food/food_list_vue.do",{
					...
					console.log(response.data)
					this.food_list=response.data //응답데이터가 food_list에 담김 jsp에서 출력 //fvo in food_list
				}).catch(error=>{
					console.log(error.response)
				})
			}
