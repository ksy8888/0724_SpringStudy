...
public class DataBoardController {
	...
	public String databoard_insert_ok(DataBoardVO vo) {
		...
		if(list==null) {  //파일이 없는 상태 (업로드가 안된경우)
			vo.setFilename("");
			vo.setFilesize("");
			vo.setFilecount(0);
	...
	//databoard/detail.do?no=${vo.no }
		 ...
		 *  			PasswordEncoder
		 *   DAO : 데이터베이스 연동
		 *   Controller : 조립기 (DAO연동 => 데이터를 추출 => 브라우저로 전송)
		...
		public String databoard_datail(int no,Model model) // DAO에서는 매개변수 여러개 jsp에서 넘어오는 변수와 매개변수 변수가 일치해야한다.
		{
			...
			if(vo.getFilecount()>0)
			{
			    ...
			    model.addAttribute("nList", nList); // 파일이 존재할떄만 보낸다
			    model.addAttribute("sList", sList); // 파일이 존재할때만 보낸다
			}
...
///////////////////////수정
		...
		@ResponseBody //내가 원하는 데이터(스크립트 등) 을 보냄 // @RestController
		...
		@ResponseBody    //PrintWriter out = response.getWriter();
