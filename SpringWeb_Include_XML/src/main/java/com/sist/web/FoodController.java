...
public class FoodController {
	...
	public String food_list(int cno,Model model) {	//${vo.cno } 무조건 값이 붙어있기 때문에 int
	...
	@GetMapping("food/food_before_detail.do")	//response가 cookie랑 html을 동시에 전송하지 못함
	public String food_before_detail(int fno, RedirectAttributes ra,HttpServletResponse response) {
		 ...
		 */
		//쿠키생성   	//Cookie(String name, String value) >> cookie는 저장이 String / Session은 Object 단위
		Cookie cookie  = new Cookie("food_"+fno, String.valueOf(fno)); 
		cookie.setPath("/");
		cookie.setMaxAge(60*60*24);
		//브라우저로 전송
		response.addCookie(cookie);
		ra.addAttribute("fno", fno); //?=fno
